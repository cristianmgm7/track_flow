...
class PlaybackStatePersistenceImpl implements PlaybackStatePersistence {
  ...
  PlaybackStatePersistenceImpl(this._prefs);

  @override
  Future<Either<Failure, Unit>> savePlaybackState(
    PersistedPlaybackState state,
  ) async {
    try {
      final prefs = _prefs;
  ...
  @override
  Future<Either<Failure, PersistedPlaybackState?>>
  restorePlaybackState() async {
    try {
      ...
      final hoursSinceUpdate =
          DateTime.now().difference(state.lastUpdated).inHours;
