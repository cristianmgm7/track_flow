...
class CacheKeyRepositoryImpl implements CacheKeyRepository {
  ...
  CacheKeyRepositoryImpl({required CacheStorageLocalDataSource localDataSource})
  ...
  Future<Either<CacheFailure, String>> getDirectoryPathFromCacheKey(
    ...
    try {
      ...
      return filePathResult.fold((failure) => Left(failure), (filePath) {
    ...
    try {
      if (storageId.contains('_')) {
        ...
        if (parts.length >= 2) {
          ...
          return Right(CacheKey.composite(trackId, checksum));
        }
      }
